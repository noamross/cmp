...
parms = list(
  ...
  mu =   0.1,
  r = 0.00001, # THIS SHOULD VARY FROM 0.2 to 0.  Bifurcation is near 0.01
  d = 0.0000,
  K = 1000,
  time_max = 150,
  prevent_inf = 0,
...
ggplot() +
  geom_line(data = filter(micro_stats_tidy, variable %in% c("cmp_kld2", "nb_kld2", "rel_kld")),
            mapping = aes(x=time, y=value, group=run), 
            col="slateblue", alpha=0.25, lwd=1) +
  geom_line(data = filter(ode_stats_tidy, variable %in% c("cmp_kld2", "nb_kld2", "rel_kld")),
            mapping = aes(x=time, y=value, group=run), 
            col="red", lwd=1) +
  geom_line(data = filter(micro_stats_mean, variable %in% c("cmp_kld2", "nb_kld2", "rel_kld")),
...
#Rprof("out.prof")
opt = nloptr_tr(x0 = c(parms$alpha), eval_f = find_underdisp,
             lb = c(0.00001), ub=c(1),
