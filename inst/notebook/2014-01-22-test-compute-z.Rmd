...
z_compute_errs = expand.grid(lambda=seq(0.1, 1.9, by=0.2), nu=seq(0.1, 1.9, by=0.2)) %>%
  group_by(lambda, nu) %>%
  mutate(z_calc = exp(com_compute_log_z(lambda, nu, log_error = 0.0000001, maxit=1000000)),
...
z_compute_errs2 = expand.grid(lambda=seq(0.01, 2.91, by=0.1), nu=seq(0.01, 2.91, by=0.1)) %>%
  group_by(lambda, nu) %>%
  mutate(log_z_calc = com_compute_log_z(lambda, nu, log_error = 0.0000001, maxit=1000000),
...
z_compute_errs3 = expand.grid(lambda=seq(0.01, 1.51, by=0.05), nu=seq(0.01, 1.51, by=0.05)) %>%
  group_by(lambda, nu) %>%
  mutate(z_calc = exp(com_compute_log_z(lambda, nu, log_error=0.00000001, maxit=1000000)),
